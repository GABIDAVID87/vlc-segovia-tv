...
class VideoTouchDelegate(private val player: VideoPlayerActivity,
    ...
    private val rightContainerId: Int by lazy { player.resources.getIdentifier("rightContainer", "id", player.packageName) }
    private val rightContainer: CircularRevealFrameLayout? by lazy {
        if (rightContainerId != 0) player.findViewById(rightContainerId) else null
    }
    private val leftContainerId: Int by lazy { player.resources.getIdentifier("leftContainer", "id", player.packageName) }
    private val leftContainer: CircularRevealFrameLayout? by lazy {
        if (leftContainerId != 0) player.findViewById(leftContainerId) else null
    }
    private val rightContainerBackgroundId: Int by lazy { player.resources.getIdentifier("rightContainerBackground", "id", player.packageName) }
    private val rightContainerBackground: HalfCircleView? by lazy {
        if (rightContainerBackgroundId != 0) player.findViewById(rightContainerBackgroundId) else null
    }
    private val leftContainerBackgroundId: Int by lazy { player.resources.getIdentifier("leftContainerBackground", "id", player.packageName) }
    private val leftContainerBackground: HalfCircleView? by lazy {
        if (leftContainerBackgroundId != 0) player.findViewById(leftContainerBackgroundId) else null
    }
    ...
    private val seekBackgroundId: Int by lazy { player.resources.getIdentifier("seek_background", "id", player.packageName) }
    private val seekBackground: FrameLayout? by lazy {
        if (seekBackgroundId != 0) player.findViewById(seekBackgroundId) else null
    }
...
// Mouse events for the core
    ...
    private fun showSeek(seekForward: Boolean): TextView {
        ...
        val seekBg = seekBackground
        if (container == null || containerBackground == null || seekBg == null) {
            // The seek overlay layout is optional. Seeking itself still works without it.
            return if (seekForward) seekRightText else seekLeftText
        }
        ...
        container.post {
            ...
            if (isTv) {
                ...
                seekTVConstraintSet.connect(rightContainerBackgroundId, ConstraintSet.START, R.id.seekRightContainer, ConstraintSet.START)
                seekTVConstraintSet.connect(rightContainerBackgroundId, ConstraintSet.TOP, R.id.seekRightContainer, ConstraintSet.TOP)
                seekTVConstraintSet.connect(rightContainerBackgroundId, ConstraintSet.BOTTOM, R.id.seekRightContainer, ConstraintSet.BOTTOM)
                ...
                seekTVConstraintSet.connect(leftContainerBackgroundId, ConstraintSet.END, R.id.seekLeftContainer, ConstraintSet.END)
                seekTVConstraintSet.connect(leftContainerBackgroundId, ConstraintSet.TOP, R.id.seekLeftContainer, ConstraintSet.TOP)
                seekTVConstraintSet.connect(leftContainerBackgroundId, ConstraintSet.BOTTOM, R.id.seekLeftContainer, ConstraintSet.BOTTOM)
            ...
            val backgroundAnim = ObjectAnimator.ofFloat(seekBg, "alpha", 1f)
            ...
            val mainAnimOut = ObjectAnimator.ofFloat(seekBg, "alpha", 0f)
            backgroundAnim.duration = 200
