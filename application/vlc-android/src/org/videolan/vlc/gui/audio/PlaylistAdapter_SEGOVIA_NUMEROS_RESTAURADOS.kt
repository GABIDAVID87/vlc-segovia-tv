...
class PlaylistAdapter(private val player: IPlayer) : DiffUtilAdapter<MediaWrapper, PlaylistAdapter.ViewHolder>(), SwipeDragHelperAdapter, SchedulerCallback {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        val segoviaSeries = player is VideoPlayerActivity &&
                player.intent.getBooleanExtra("from_external", false) &&
                player.intent.getStringExtra("content_type").equals("series", ignoreCase = true)
        // Segovia TV: nunca mostrar el MRL/URL del proxy en la playlist.
        // La duración tampoco es necesaria aquí; el panel lateral muestra solo el capítulo.
        holder.binding.subTitle = if (segoviaSeries) "" else MediaUtils.getMediaSubtitle(media)
        ...
        holder.binding.showTrackNumbers = if (segoviaSeries) true else showTrackNumbers
        holder.binding.selector.isSelected = currentIndex == position
        ...
        if (segoviaSeries) holder.binding.itemDelete.setGone()
        else if (tablet) holder.binding.itemDelete.setVisible() else holder.binding.itemDelete.setGone()
        holder.binding.showReorderButtons = if (segoviaSeries) false else showReorderButtons && tablet

        holder.binding.executePendingBindings()

        if (segoviaSeries) {
            // SEGOVIA TV: el número de episodio debe quedar siempre visible.
            // Se fuerza después de executePendingBindings() para que el DataBinding
            // del layout no vuelva a ocultarlo al reciclar la fila.
            holder.binding.trackNumber.visibility = View.VISIBLE
            holder.binding.trackNumber.text = String.format(java.util.Locale.ROOT, "%02d", position + 1)
            holder.binding.audioItemTitle.text = segoviaEpisodeTitle(media.title?.toString().orEmpty())
        } else {
            holder.binding.trackNumber.visibility = if (showTrackNumbers) View.VISIBLE else View.GONE
        }
    }
    ...
    private fun segoviaEpisodeTitle(raw: String): String {
        val value = raw.trim()
        ...
        return match?.groupValues?.getOrNull(1)?.trim()?.takeIf { it.isNotBlank() }
            ?: value.substringAfterLast(" - ").trim().ifBlank { value }
    }
