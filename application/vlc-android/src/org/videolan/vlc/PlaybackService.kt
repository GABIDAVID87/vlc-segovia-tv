...
class PlaybackService : MediaBrowserServiceCompat(), LifecycleOwner, CoroutineScope, SchedulerCallback {
    ...
    override fun onGetRoot(clientPackageName: String, clientUid: Int, rootHints: Bundle?): BrowserRoot? {
        ...
        // ============================================================
        // BLOQUE SEGOVIA TV — permitir MediaBrowser sin almacenamiento
        // Segovia TV solo necesita leer la MediaSession de RCTV para
        // obtener capítulo, posición, duración y cola.
        // ============================================================
        val isSegoviaClient =
                clientPackageName == "com.segovia.tv" ||
                clientPackageName == "com.segovia.tv.debug"

        if (!isSegoviaClient && !Permissions.canReadStorage(ctx)) {
            Log.w(TAG, "Returning null MediaBrowserService root. READ_EXTERNAL_STORAGE permission not granted.")
            return null
        }

        if (isSegoviaClient) {
            Log.i(TAG, "Segovia TV conectado al MediaBrowser: $clientPackageName")
        }
